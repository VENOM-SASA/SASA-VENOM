...
@Client.on_message(command(["صوت", f"song@{bn}"]) & ~filters.edited)
def song(_, message):
...
@Client.on_message(
    command(["صوت", f"vsong@{bn}", "فيديو", f"video@{bn}"]) & ~filters.edited
)
...
@Client.on_message(command(["الكلمات", f"lyric@{bn}"]))
